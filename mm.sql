...
select count( * ) into num_rows from (
  ...
  D.data_wystawienia
    ...
    and PZR.POZ_ID IS NULL
    AND DATE (d.data_wystawienia) >= '2023/01/01'
    AND DATE (d.data_wystawienia) <= '2023/12/31'
    GROUP BY
        poz.poz_id,  a.prefiks_indeks, d.numer, D.data_wystawienia) x;
...
AND DATE (d.data_wystawienia) >= '2023/01/01'
AND DATE (d.data_wystawienia) <= '2023/12/31'
AND poz.poz_id NOT IN (SELECT poz_id FROM temp_poz_id)
GROUP BY
poz.poz_id,poz.ilosc, a.prefiks_indeks, d.numer, D.data_wystawienia;
